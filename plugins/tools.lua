--Begin Tools.lua :)
local SUDO = 000000 -- ایدی عددی سودو! <===
...
local function pre_process(msg)
	if msg.to.type ~= 'pv' then
		...
		if chex and not exd and msg.from.id ~= SUDO and not is_sudo(msg) then
			...
			if lang then
				tdcli.sendMessage(-1001085115300, 0, 1, text1, 1, 'html')
				tdcli.sendMessage(msg.to.id, 0, 1, text2, 1, 'md')
			else
				tdcli.sendMessage(-1001085115300, 0, 1, text3, 1, 'md')
...
if ((matches[1] == "clear cache" and not Clang) or (matches[1] == "پاک کردن حافظه" and Clang)) then
    ...
    return "_⚙️حافظه کش شده سرور با موفقیت پاکسازی گردید_"
   end
...
if is_sudo(msg) then
   		if ((matches[1]:lower() == 'add' and not Clang) or (matches[1] == "افزودن" and Clang)) and not redis:get('ExpireDate:'..msg.to.id) then
			...
			redis:setex('ExpireDate:'..msg.to.id, 180, true)
				...
				if lang then
					local text1 = '👥گروه جدیدی به لیست مدیریتی ربات ربات (2) افزوده گردید \n\n🆔آیدی گروه:  <code>'..msg.to.id..'</code>\n\nدر صورتی که میخواهید ربات این گروه را ترک کند از دستور زیر استفاده کنید\n\n/leave '..msg.to.id..'\nبرای جوین دادن توی این گروه میتونی از دستور زیر استفاده کنی:\n/jointo '..msg.to.id..'\n_________________\nدر صورتی که میخواهید گروه رو دوباره شارژ کنید میتوانید از کد های زیر استفاده کنید...\n\n<b>برای شارژ 1 ماهه:</b>\n/plan 1 '..msg.to.id..'\n\n<b>برای شارژ 3 ماهه:</b>\n/plan 2 '..msg.to.id..'\n\n<b>برای شارژ نامحدود:</b>\n/plan 3 '..msg.to.id
						tdcli.sendMessage(-1001085115300, 0, 1, text1, 1, 'html')
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '‼️توجه : ربات به مدت 3 دقیقه در این گروه فعال شد لطفا تا 3 دقیقه تنظیمات را انجام داده و شارژ گروه را هر چه سریعتر وارد نمایید در غیر اینصورت ربات لفت خواهد داد‼️', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_‼️Group charged 3 minutes  for settings‼️_', 1, 'md')
				end
		end
		...
		if ((matches[1] == 'leave' and not Clang) or (matches[1] == "خروج" and Clang)) and matches[2] then
			if lang then
				...
				tdcli.sendMessage(-1001085115300, msg.id_, 1, 'ربات با موفقیت از گروه '..matches[2]..' خارج شد.', 1,'md')
			else
		...
		if string.match(matches[2], '^-%d+$') then
			if tonumber(matches[3]) > 0 and tonumber(matches[3]) < 1001 then
				...
				if lang then
					tdcli.sendMessage(-1001085115300, 0, 1, 'ربات در گروه '..matches[2]..' به مدت '..matches[3]..' روز تمدید گردید.', 1, 'md')
		...
		if ((matches[1] == 'charge' and not Clang) or (matches[1] == "شارژ" and Clang)) and matches[2] and not matches[3] and is_sudo(msg) then
			if tonumber(matches[2]) > 0 and tonumber(matches[2]) < 1001 then
				...
				if lang then
					...
					tdcli.sendMessage(-1001085115300, 0, 1, 'ربات در گروه '..matches[2]..' به مدت `'..msg.to.id..'` روز تمدید گردید.', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, 'ربات با موفقیت تنظیم شد\nمدت فعال بودن ربات در گروه به '..matches[2]..' روز دیگر تنظیم شد...', 1, 'md')
					tdcli.sendMessage(-1001085115300, 0, 1, 'ربات در گروه '..matches[2]..' به مدت `'..msg.to.id..'` روز تمدید گردید.', 1, 'md')
				end
...
if ((matches[1] == "adminprom" and not Clang) or (matches[1] == "ادمین" and Clang)) and is_admin(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if ((matches[1] == "admindem" and not Clang) or (matches[1] == "حذف ادمین" and Clang)) and is_admin(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if ((matches[1] == 'modern' and not Clang) or (matches[1] == "ربات" and Clang)) then
...
_Sudoer And Admins modern Bot Help :_
...
text = [[
_راهنمای ادمین و سودو های ربات ربات:_
