...
tdcli_function ({
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') and not msg.reply_id then
   tdcli_function ({
	...
	local list = 'لیست کسانی که تا کنون به آنها اخطار داده اید:\n'
local empty = list
		...
		list = list..k..'- '..check_markdown(user_info.user_name)..' [`'..v..'`] *تعداد اخطار های فرد : '..(cont - 1)..'*\n'
       else
		list = list..k..'- `'..v..'` *تعداد اخطار های فرد : '..(cont - 1)..'*\n'
      end
    end
		if list == empty then
		return '_تا کنون هیچ اخطاری در گروه به کسی نداده اید_'
		else
		return list
		end
	end
